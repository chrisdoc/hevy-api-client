...
async function extractHevyOpenApiSpec() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch Swagger UI init script: ${response.status} ${response.statusText}`,
      );
    }
    ...
    const specMatch = scriptContent.match(
    ...
    if (!specMatch) {
      ...
      throw new Error(
        "Could not find OpenAPI spec in Swagger UI init script. The format may have changed.",
      );
    }
    ...
    try {
      ...
      console.error(
        "Failed to evaluate extracted JavaScript object:",
        evalError.message,
      );
      console.log(
        "First 500 characters of extracted string:",
        specString.substring(0, 500),
      );
      throw new Error(
        `Invalid JavaScript object in OpenAPI spec: ${evalError.message}`,
      );
    }
    ...
    if (!spec.openapi && !spec.swagger) {
      throw new Error(
        "Extracted object is not a valid OpenAPI/Swagger specification",
      );
    }
    ...
    if (!spec.paths) {
      console.warn(
        '⚠️  OpenAPI spec missing "paths" section - no API endpoints found',
      );
    }

    console.log(
      "✅ Successfully extracted OpenAPI specification from live API",
    );
    console.log(
      `📊 Found ${Object.keys(spec.paths || {}).length} API endpoints`,
    );
    console.log(
      `🔧 Found ${Object.keys(spec.components?.schemas || {}).length} schema definitions`,
    );
    ...
    if (error.message.includes("fetch") || error.message.includes("timeout")) {
      console.error(
        "   💡 This might be a network connectivity issue or the endpoint is down.",
      );
      console.error(
        "   💡 Try visiting https://api.hevyapp.com/docs/ in your browser to verify.",
      );
    } else if (
      ...
      console.error(
        "   💡 The response format may have changed. Consider updating the extraction logic.",
      );
    }
  }
}
...
async function main() {
  try {
    ...
    console.log(
      `📊 Spec contains ${Object.keys(spec.paths || {}).length} API endpoints`,
    );
    console.log(
      `🔧 Schema definitions: ${Object.keys(spec.components?.schemas || {}).length}`,
    );
  } catch (error) {
