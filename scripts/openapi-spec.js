...
function fixOpenAPISpec(spec) {
  ...
  if (!fixed.servers || fixed.servers.length === 0) {
    fixed.servers = [{ url: "https://api.hevyapp.com", description: "Hevy API" }];
...
function fixSchemaRequired(schema, path = "") {
  ...
  if (schema.additionalProperties && typeof schema.additionalProperties === "object") {
    fixSchemaRequired(schema.additionalProperties, `${path}.additionalProperties`);
  }
}
...
function fixInvalidExamples(schemas) {
  ...
  if (routine?.properties?.exercises?.items?.properties?.rest_seconds) {
    const restSeconds = routine.properties.exercises.items.properties.rest_seconds;
    if (typeof restSeconds.example === "number") {
      restSeconds.example = String(restSeconds.example);
      console.log("  Fixed: schemas.Routine...rest_seconds - converted example to string");
    }
    // API returns number, spec says string - allow both for compatibility
    if (restSeconds.type === "string") {
      restSeconds.oneOf = [{ type: "string" }, { type: "number" }];
      delete restSeconds.type;
      console.log(
        "  Fixed: schemas.Routine...rest_seconds - allow string or number (API returns number)",
      );
    }
  }
  // Allow notes to be null (API sometimes returns null)
  const routineExercises = schemas.Routine?.properties?.exercises?.items?.properties;
  if (routineExercises?.notes) {
    if (!routineExercises.notes.nullable) {
      routineExercises.notes.nullable = true;
      console.log("  Fixed: schemas.Routine...notes - allow null (API returns null)");
    }
  }
}
    ...
    for (const method of ["get", "post", "put", "patch", "delete"]) {
      ...
      const pathParts = pathName
        ...
        .map((part, i) => {
          ...
          return i === 0 ? camelCase : camelCase.charAt(0).toUpperCase() + camelCase.slice(1);
        })
        .join("");

      operation.operationId = method + pathParts.charAt(0).toUpperCase() + pathParts.slice(1);
      console.log(
